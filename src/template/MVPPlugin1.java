...
public class MVPPlugin1 extends AnAction {
    Project project;
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        if (className.equals("mvp") || className.equals("MVP") || className.equals("Mvp")) {
            ...
            String path0 = selectGroup.getPath();
            if (path0.endsWith("mvp")) {
                mvpPath = path0.substring(path0.indexOf("java") + 5, path0.length()).replace("/", ".");
            }else if (mvpPath ==""){
                Messages.showInfoMessage(project, "请先选择基库mvp文件后调用alt+m 快捷健", "警告");
            }else {
    ...
    private void createClassMvp(String className) {
        ...
        String packageName = path.substring(path.indexOf("java") + 5, path.length()).replace("/", ".");
        if (mvpPath == null ||mvpPath =="") {
            mvpPath = FileUtil.traverseFolder(path.substring(0, path.indexOf("java")));
